...
import {
    ...
    CreateButtonWrapper,
    CreateButton,
} from './userMilkshakes.styles';

interface UserMilkshakesProps {
    userMilkshakes: FindUserMilkshakesResponse;
}

const UserMilkshakes: FunctionComponent<UserMilkshakesProps> = ({
    userMilkshakes,
}) => {
    const fetcher = (query: RequestDocument) =>
        graphqlClient.request(query, { limit: 3 });
            ...
            <Wrapper className="wrapper">
                <ContentWrapper>
                    {data?.findUserMilkshakes?.map((milkshake) => (
                        <Link
                            key={milkshake.id}
                            href={`/milkshake/${milkshake.id}`}
                        >
                            <MilkshakeWrapper className="milkshake">
                                <MilkshakeIconWrapper>
                                    <svg
                                        width="72"
                                        height="110"
                ...
                <CreateButtonWrapper width="80%" color="secondary">
                    <Link href="/create-milkshake" passHref>
                        <CreateButton color="secondary">Create</CreateButton>
                    </Link>
                </CreateButtonWrapper>
            </Wrapper>
        </BorderWrapper>
